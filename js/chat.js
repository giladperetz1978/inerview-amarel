...
(() => {
  ...
  function generateSmartLocalResponse(query, actionType = null) {
    ...
    if (actionType === "prep" || q.includes("תסריט") || q.includes("הכנ")) {
      let res = `**🎯 תסריט מומלץ לראיון גיוס באמרל:**\n\n`;
      res += `1. **פתיחה (5 דקות):** הצגת המשרה, המחלקה, ותיאום ציפיות לגבי מבנה הראיון.\n`;
      res += `2. **ניסיון מקצועי (15 דקות):** מעבר על פרויקטים מרכזיים, טכנולוגיות ואתגרים טכניים שעמד/ה בהם.\n`;
      res += `3. **סיטואציות ומקרים (15 דקות):** התמודדות עם קונפליקטים בצוות, עבודה תחת עומס ולמידה מטעויות.\n`;
      res += `4. **שאלות המועמד/ת והמשך תהליך (10 דקות):** מתן מקום לשאלות והסבר על השלבים הבאים.\n`;
      return { text: res, extracted: null };
    }
